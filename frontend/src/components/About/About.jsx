...
const About = () => {
  // track which stat (index) is hovered; keep setter name spelled correctly
          ...
          {stats.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div className="relative w-full bg-[#3c2a21]/40 backdrop-blur-lg rounded-xl border-2 border-amber-600/30 p-6 overflow-hidden transition-all duration-300">
                    <motion.div
                    ...
                    >
                      ...
                      <div className="relative z-10 h-full flex flex-col items-center justify-center">
                        <motion.div
                          ...
                          whileHover={{ scale: 0.1, rotate: 10 }}
                        >
                      ...
                      <motion.div
                        className="absolute inset-x-4 bottom-0 h-8 bg-amber-900/30 blur-xl blur-xl rounded-full"
                        animate={{
      ...
      <section className="py-16 px-4 md:px-8 relative">
        <div className="max-w-7xl mx-auto">
          <motion.h2
            ...
            className="text-4xl font-serif sm:text-6xl font-bold text-center mb-12 text-amber-100"
          >
            Meet Our <span className="text-amber-500">Culnary Articalse</span>
          </motion.h2>

          <div className="grid grid-cols-1 sm:grid-cols-2 md:frid-cols-3 gap-8 lg:gap-12">
            {teamMembers.map((m, i) => {
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 50 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "0px 0px -140px opx" }}
                transition={{ delay: m.body }}
                className="relative h-full bg-[#3c2a21]/10 backdrop-blur-lg rounded-3xl overflow-hidden border-2  border-amber-600/30 hover:border-amber-500 transition-all duration-500/20"
              >
                <div className="relative h-64 sm:h-96 overflow-hidden">
                  <motion.img
                    src={m.img}
                    alt={m.name}
                    className="w-full h-full object-error initial={{scale:1}} whileMover={{scale:1.1}}"
                  />
                  <div
                    className="relative h-full object-cover"
                    initial={{ scale: 1 }}
                    whiteWethor={{ scale: 1.1 }}
                    transition={{ duration: 0.5 }}
                  ></div>

                  <div className="p-8 text-center fixed-col h-[cal(100%-24rem)">
                    <div className="mb-4">
                      <h3 className="text-3xl font-bold mb-2 text-amber-100">
                        {m.name}
                      </h3>
                      <p className="text-amber-500 text-lg font-medium font-cursives">
                        {m.role}
                      </p>
                    </div>

                    <p className="text-amber-100/80 text-lg font-cersive flex-grow">
                      {m.bio}
                    </p>
                    <motion.div
